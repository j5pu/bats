...
# <a href="https://bats-core.readthedocs.io/en/stable/writing-tests.html#special-variables">$BATS_FILE_TMPDIR</a>
# </html>
export BATS_TMP
...
_bats::run() {
  ...
  local tests=("${BATS_TESTS}")

  rm -rf "${output}"

  if [ "${1-}" ]; then
    case "${1-}" in
      commands) printf '%s\n' "$1" list run verbose; exit ;;
...
_bats::vars() {
  ...
  if [ "${BATS_TEST_DIRNAME-}" ]; then
    if [[ "${BATS_TEST_DIRNAME##*/}" =~ test|tests|__tests__ ]]; then
      BATS_TESTS="${BATS_TEST_DIRNAME}"
      return
    fi
    dirname="$(dirname "${BATS_TEST_DIRNAME}")"
    if [[ "${dirname##*/}" =~ test|tests|__tests__ ]]; then
      BATS_TESTS="${dirname}"
      return
    fi
    : "${BATS_TESTS?No Test Directory found from "${BATS_TEST_DIRNAME}"}"
  else
